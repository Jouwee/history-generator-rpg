...
impl MatrixIndex {
    ...
    pub(crate) fn neighbours8(&self, x: usize, y: usize) -> Vec<usize> {
        let mut vec = vec!();
    ...
    pub(crate) fn p2d(&self, point: Point2D) -> usize {
        return (point.1 * self.size.0) + point.0;
    }

    pub(crate) fn to_p2d(&self, idx: usize) -> Point2D {
        let y = idx / self.size.0;
        return Point2D(idx % self.size.0, y);
    }

    pub(crate) fn p2d_neighbours8(&self, point: Point2D) -> Vec<usize> {
