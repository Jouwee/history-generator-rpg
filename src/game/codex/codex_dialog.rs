use crate::{commons::{history_vec::Id, id_vec::Id as VId}, engine::{gui::{button::{Button, ButtonEvent}, container::Container, dialog::Dialog, label::Label, vlist::VList, Anchor, GUINode, Position}, render::RenderContext}, resources::resources::Resources, world::{creature::CreatureId, world::{ArtifactId, World}}, GameContext};
...
impl CodexDialog {
    ...
    pub(crate) fn input_state(&mut self, evt: &crate::game::InputEvent, world: &World, resources: &Resources, codex: &KnowledgeCodex) {
        if let Some(dialog) = &mut self.dialog {
            ...
            if let ButtonEvent::Click = dialog.get_mut::<Button>("btn_creatures").unwrap().event(evt) {
                ...
                Self::click_creatures(dialog.get_mut::<VList>("entry_list").unwrap(), world, resources, codex);
                return
            }
            ...
            if let ButtonEvent::Click = dialog.get_mut::<Button>("btn_artifacts").unwrap().event(evt) {
                ...
                Self::click_artifacts(dialog.get_mut::<VList>("entry_list").unwrap(), world, resources, codex);
                return
            }
            if let View::Creatures = self.view {
                for (id, _knowledge) in codex.known_creatures() {
                    if let ButtonEvent::Click = dialog.get_mut::<VList>("entry_list").unwrap().get_mut::<Button>(format!("creature:{}", id.as_usize()).as_str()).unwrap().event(evt) {
                        Self::click_creature(dialog.get_mut::<VList>("selected_info").unwrap(), id, world, resources, codex);
                    }
                }
            }
            if let View::Places = self.view {
                for (id, _knowledge) in codex.known_places() {
                    if let ButtonEvent::Click = dialog.get_mut::<VList>("entry_list").unwrap().get_mut::<Button>(format!("place:{}", id.0).as_str()).unwrap().event(evt) {
                        Self::click_place(dialog.get_mut::<VList>("selected_info").unwrap(), *id, world, resources, codex);
                    }
                }
            }
            if let View::Artifacts = self.view {
                for (id, _knowledge) in codex.known_artifacts() {
                    if let ButtonEvent::Click = dialog.get_mut::<VList>("entry_list").unwrap().get_mut::<Button>(format!("artifact:{}", id.as_usize()).as_str()).unwrap().event(evt) {
                        Self::click_artifact(dialog.get_mut::<VList>("selected_info").unwrap(), *id, world, resources, codex);
                    }
                }
            }
    ...
    fn click_creatures(container: &mut VList, world: &World, resources: &Resources, codex: &KnowledgeCodex) {
        ...
        for (id, _knowledge) in codex.known_creatures() {
            ...
            container.add_key(format!("creature:{}", id.as_usize()).as_str(), Button::new(format!("{:?}", id), Position::Auto));
        }
    }
    ...
    fn click_creature(container: &mut VList, id: &CreatureId, world: &World, resources: &Resources, codex: &KnowledgeCodex) {
        // TODO:
        // let creature = world.creatures.get(id);
        // let knowledge = codex.creature(&id).unwrap();
        // let writer = BiographyWriter::new(world, resources);
        container.clear();
        container.add(Label::new(format!("{:?}", id), Position::Auto));
