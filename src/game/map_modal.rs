...
impl MapModal {
    ...
    pub fn render(&mut self, ctx: &mut RenderContext, game_ctx: &mut GameContext) {
        ...
        if cursor != cursor_clamp {
            let icon = game_ctx.assets.image("map_tiles/player_offscreen.png");    
            ctx.texture_ref(&icon.texture, cursor_clamp);
        } else {
            let icon = game_ctx.assets.image("map_tiles/player.png");
            ctx.texture_ref(&icon.texture, cursor_clamp);
        }
        let _ = ctx.try_pop();
        // Control
        let icon = game_ctx.assets.image("controls/right_click.png");
